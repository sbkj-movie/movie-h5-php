...
//        }
		...
		if(!empty($list)){
			foreach($list as $key=>$val){
                ...
                if(strstr($val['CT_PHYLETIC'],',') !=false){
                    $class_id = explode(',',$val['CT_PHYLETIC']);
                    $wn['ID']=array('in',$class_id);
                    $types =M('mv_class')->where($wn)->getField('CL_NAME',true);
                    $v['CT_PHYLETIC']=implode(',',$types);
                }else{
                    $v['CT_PHYLETIC'] =M('mv_class')->where("ID=".$val['CT_PHYLETIC'])->getField('CL_NAME');
                }
	...
	{   
		...
		if(isset($id)&&$id!=0){
            ...
            if(strstr($data['CT_PHYLETIC'],',') !=false){
                $data['ct_id']=explode(',',$data['CT_PHYLETIC']);
            }
		}
		//分类
	...
	//管理员提交
	 function doadd(){
		...
		$data['CT_PHYLETIC']=implode(",", $_POST['classid']);
