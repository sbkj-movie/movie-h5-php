			   ...
			   if ($_FILES["excel"]["error"] > 0)
				...
				else
				  {
					  ...
					  move_uploaded_file($_FILES["excel"]["tmp_name"],$path);
						...
						//添加表格
						$in="insert  into `cm_import`(`path`,`name`,`is_daoru`,`daorutime`,`addtime`) values ('{$path}','$name',1,$addtime,$addtime)";
			 //echo $sql;
						$results = mysqli_query($link,$in);
						$id=mysqli_insert_id($link);
						$gongzi=inserExcel($path);
						
						$msg=dopostall($gongzi,$id);
...
function dopostall($arr,$id){
	...
	if(empty($arr)){
		...
		foreach($arr as $val){
			...
			$sql="insert  into `cm_gongzi`(`idcard`,`name`,`gangwei`,`xinji`,`jichu`,`weisheng`,`huling`,`jintie`,`jiangli`,`huifei`,`xiujia`,`boy`,`jinbu`,`wuye`,`shujia`,`zhideng`,`jixiao`,`fengxian`,`bucha`,`jiaotong`,`tongxun`,`koukuan`,`yingfu`,`yingshui`,`zhufang`,`yanglao`,`ylbx`,`baoxian`,`qiye`,`jishu`,`geren`,`shifa`,`benci`,`danwei`,`yiliao`,`shengyu`,`gongshang`,`shiye`,`gongjijin`,`zhanghu`,`qunuan`,`teshu`,`jingshen`,`mubiao`,`qita`,`year`,`month`,`addtime`,`imid`) values ($val[0],'$val[1]',$val[2],$val[3],$val[4],$val[5],$val[6],$val[7],$val[8],$val[9],$val[10],$val[11],$val[12],$val[13],$val[14],$val[15],$val[16],$val[17],$val[18],$val[19],$val[20],$val[21],$val[22],$val[23],$val[24],$val[25],$val[26],$val[27],$val[28],$val[29],$val[30],$val[31],$val[32],$val[33],$val[34],$val[35],$val[36],$val[37],$val[38],$val[39],$val[40],$val[41],$val[42],$val[43],$val[44],$val[45],$val[46],$addtime,$id)";
			 //echo $sql;die();
